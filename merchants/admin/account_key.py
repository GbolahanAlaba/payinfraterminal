...
class MerchantAccountKeyInline(admin.StackedInline):
    ...
    can_delete = False
    max_num = 1
    readonly_fields = ('merchant', 'created_at', 'updated_at')
    fields = ('account_id', 'merchant', 'environment', 'is_active', 'created_at', 'updated_at')


@admin.register(MerchantAccountKey)
class MerchantAccountKeyAdmin(admin.ModelAdmin):
    list_display = ('merchant', 'account_id', 'environment', 'is_active', 'created_at')
